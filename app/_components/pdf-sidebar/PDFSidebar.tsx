...
const PDFSidebar = () => {
  ...
  const pdfFilesList = Array(30).fill('_').map((_, i) => (
    <Button key={i} variant="ghost" className="w-full flex items-center justify-start">
      <FcDocument className="mr-2 h-4 w-4" />
      <span className={
        cn(resolvedTheme !== 'dark' && 'text-black')
      }>
        PDF Document {i}
      </span>
    </Button>
  ...
  return (
    <div className="max-w-[20vw] flex h-full fixed">
      <div className="h-full w-[20vw] max-w-[20vw] flex flex-col pl-4">
        ...
        <div className="flex pt-[1vh] pb-[2vh] overflow-x-hidden">
          <ul role="list" className="divide-y divide-gray-100">
            {people.map((person) => (
              <li key={person.email} className="flex justify-between gap-x-4 py-5 pr-2">
